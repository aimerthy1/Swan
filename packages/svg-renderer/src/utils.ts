...
export const applyAttributes = (element: SVGElements, attributes: Attributes) => {
...
export const applyTransform = (element: SVGElements, transform: string) => {
  const currentTransform = element.getAttribute('transform') ? `${element.getAttribute('transform')} ` : ''
